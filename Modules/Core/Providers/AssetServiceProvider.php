...
use Modules\Core\Foundation\Asset\Manager\azshopAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\azshopAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, azshopAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new azshopAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("azshop.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("azshop.modules.{$module->getAlias()}.assets");
